    ...
    class ShapeMethods
    {
            ...
            if (ShapeList.Count != 0)
            {
                for (int i = 0; i < ShapeList.Count; i++)
                {
                    ...
                    if (ShapeList[i] is Circle)
                    {
                        ...
                        BeautConsole.WriteLineColor($"   Radius: {ShapeList[i].GetParam("Radius")}", ConsoleColor.DarkYellow);
                    }
                    else if (ShapeList[i] is Rectangle)
                    {
                        BeautConsole.WriteLineColor($"{i + 1})Rectangle", ConsoleColor.Yellow);
                        BeautConsole.WriteLineColor($"   Side A : {ShapeList[i].GetParam("A")}\n   Side B : {ShapeList[i].GetParam("B")}", ConsoleColor.DarkYellow);
                    }
                    else if (ShapeList[i] is Square)
                    {
                        BeautConsole.WriteLineColor($"{i + 1})Square", ConsoleColor.Yellow);
                        BeautConsole.WriteLineColor($"   Side : {ShapeList[i].GetParam("Side")}", ConsoleColor.DarkYellow);
                    }
                    else if (ShapeList[i] is Triangle)
                    {
                        BeautConsole.WriteLineColor($"{i + 1})Triangle", ConsoleColor.Yellow);
                        BeautConsole.WriteLineColor($"   Side A : {ShapeList[i].GetParam("A")}\n   Side B : {ShapeList[i].GetParam("B")}\n   Side C : {ShapeList[i].GetParam("C")}", ConsoleColor.DarkYellow);
                    }
                    WriteLine();
                }
            }
        ...
        public void OutputPerimetrShape()
        {
            ...
            try
            {
                ...
                {
                    BeautConsole.WriteLineColor($"The perimetr of the {figureNumb}th rectangle: {ShapeList[figureNumb].Perimeter()}", ConsoleColor.DarkCyan);
                }
                else if (ShapeList[figureNumb - 1] is Square)
                {
                    BeautConsole.WriteLineColor($"The perimetr of the {figureNumb} th square: {ShapeList[figureNumb].Perimeter()}", ConsoleColor.Blue);
                }
                else if (ShapeList[figureNumb - 1] is Triangle)
                {
                    BeautConsole.WriteLineColor($"The perimetr of the {figureNumb} th triangle: {ShapeList[figureNumb].Perimeter()}", ConsoleColor.DarkGreen);
                }
                ReadKey();
            }
            catch (Exception)
